...
public class FileUtils {
    ...
    public static boolean isFileExisting(String fileName) {
        return isFileExisting(Environment.getExternalStorageDirectory().getPath(), fileName);
    }

    private static boolean isFileExisting(String homeDir, String fileName) {
        File file = getFile(homeDir, fileName);
        return file != null && file.exists();
    }
    
    private static File getFile(String homeDir, String fileName) {
        if (!fileName.isEmpty() && isSDCardMounted()) {
            String filePath = homeDir + File.separator + fileName;
            Log.e("cyTest", "filePath :"+filePath);
